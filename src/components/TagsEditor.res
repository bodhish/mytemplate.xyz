...
let add = (updateTagCB, selectedTags, tag) => updateTagCB(Array.append(selectedTags, [tag]))

let removeTag = (collection, i, updateTagCB, send) => {
  updateTagCB(collection |> Js.Array.filteri((_a, index) => index != i))
  send("")
}

@react.component
let make = (~unselectedTags=[], ~selectedTags, ~updateTagsCB, ~allowNewTags=true) => {
  let addTagCB = add(updateTagsCB, selectedTags)
        ...
        |> Js.Array.mapi((tag, index) =>
          <div
            ...
            <span
              ...
              onClick={_e => removeTag(selectedTags, index, updateTagsCB, send)}>
